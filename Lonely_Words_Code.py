...
letter_scores:dict = {'a':1,'b':5,'c':3,'d':2,'e':1,'f':4,'g':4,'h':2,'i':1,'j':6,'k':1,'l':2,'m':3,'n':1,'o':1,'p':1, 
                      'q':10,'r':2,'s':1,'t':1,'u':3,'v':5,'w':5,'x':6,'y':5,'z':10}

all_letters_csv = pd.read_csv("english Dictionary.csv")
all_letters_csv = pd.Series(all_letters_csv['word'], copy=True).str.lower()
...
small_text = pygame.font.Font('MedodicaRegular.otf', 40)
big_text = pygame.font.Font('MedodicaRegular.otf', 80)
smaller_text = pygame.font.Font('MedodicaRegular.otf', 20)
...
# calculates the score of a word that is entered. Based on Scrabble's scoring system
def calculate_score(word) -> int:
    score_of_word: int = 0
    for character in word:
        score_of_word += letter_scores[character]
            ...
            match sprite.get_destination():
                ...
                case 'RESHUFFLE':
                    if reshuffles_left > 0:
                        ...
                        reshuffles_left -= 1
                        sprite.set_title(f'Reshuffle Left: {reshuffles_left}')
                case _:
                    current_word = current_word.lower()
                    if current_word in all_current_words:
                        notification = "You already Entered this word!"
                    elif not(all_letters_csv.eq(current_word).any()):
